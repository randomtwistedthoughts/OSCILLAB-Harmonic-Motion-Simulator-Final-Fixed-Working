...
(() => {
    ...
    function drawSimulation(m) {
        ...
        const cy = H * 0.56;
        const centerX = W / 2;
        // The wall is FIXED. Only the mass moves back and forth.
        // Keep enough room between the wall and the leftmost particle position
        // so the spring never collapses into/through the wall.
        const wallX = Math.max(70, Math.min(120, W * 0.12));
        const maxTravel = Math.max(40, Math.min(W * 0.30, centerX - wallX - 75, W / 2 - 80));
        ...
        // Fixed wall + variable-length spring
        // IMPORTANT: wallX is deliberately independent of particleX.
